...
<script>
//var htmlstructure = '<span style="font-weight: bold;font-size: 1.4rem">{Setting Name}</span><div class="ts divider"></div><div class="ts grid" style="font-size: 1.14286rem;"><div class="eight wide column" style="text-align: right;">{Description}</div><div class="eight wide column" style="text-align: left;">{Item}<br></div></div>';
var htmlstructure = '<span style="font-weight: bold;font-size: 1.4rem">{Setting Name}</span><div class="ts divider"></div><div class="ts grid" style="font-size: 1.14286rem;"><div class="eight wide column" style="text-align: right;">{Description}</div>{Item}</div>';

$.get( "opr.php?filename=../DIsplayName.mobileconfig", function( str ) {
	...
	$.each( data["PayloadContent"], function( key, value ) {
		...
		$.each( value, function( SubKey, SubValue ) {
			if(!Array.isArray(SubValue) && !isObject(SubValue) && !SubKey.includes("Payload")){
				tmpDescription = tmpDescription + '<br><span style="color:#b5b7b9" attr="subkey" properties="' + value["PayloadType"] + '" key="' + SubKey + '">' + SubKey + ':&nbsp;</span>';
				tmpItem = tmpItem + "<br>" + SubValue;
			}else if(!SubKey.includes("Payload")){
				tmpDescription = tmpDescription + '<br><span style="color:#b5b7b9" attr="subkey" properties="' + value["PayloadType"] + '" key="' + SubKey + '">' + SubKey + ':&nbsp;</span>';
				tmpItem = tmpItem + "<br>" + JSON.stringify(SubValue);
			}
		});
